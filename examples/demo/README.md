...
- The **gate board** for the active ticket (`ADT-124`) under the `regulated` preset — note `CODE_REVIEWED` is **rejected** (the review caught a token-TTL / rate-limit issue), `SECOPS_APPROVED` carries the `safety-override`, and the rest are passed/pending.
