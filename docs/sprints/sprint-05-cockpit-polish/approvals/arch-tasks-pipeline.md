...
- `workflowView.stages` is an **ordered** list (the active track flattened by `projectWorkflowView()`). "First stage" = `stages[0]`. The **done stage is name-driven, not purely positional** — `doneStage()` returns the first stage whose name is a conventional done token, falling back to `stages[stages.length - 1]` only when none match (so a stage placed after done does not steal the folder). These are facts already in the FE.
...
> **"Done" = tickets at the done stage**, resolved by `doneStage(workflowView)`: the **first** stage whose name is a conventional done token (`done`/`complete`/`completed`/`closed`/`shipped`/`released`, case-insensitive), falling back to the **last** stage only when no stage carries such a name. This is deliberately *not* blindly the last stage — a workflow may legitimately place a stage **after** done (e.g. a trailing `Test`/audit pass), and that trailing stage stays a normal column rather than stealing the folder. (`statusOf()` already maps stage `done` → status `done`.) The **stacked, clickable folder is pure FE presentation**: the done stage column is rendered as a collapsed folder that stacks its N done cards behind a folder face, expands in place (or in a focus-trapped sheet) on click/Enter/Space, and shows a live count.

- **No new backend.** The folder collapses cards the board *already* places in the done stage column; the count is `doneTickets.length` (the set `partitionBoard()` routes to `doneStage`). Increment animations and the count are derived locally.
- **Threshold:** render the folder for the done stage **unconditionally** (a folder of 0 still reads as "nothing shipped yet"; a folder of 1 still reads as a folder) — this keeps the terminus visually stable and sticky at the right edge regardless of throughput. (Apex §2.6 recommended *not* building a separate done-folder unless the column crowds; Aura's brief explicitly asks for the folder. The folder **is** the done stage column, just collapsed — so it is the same data with a different chrome, not a new concept. `/po` owns the "always a folder vs only above N" product call; architecturally either is a pure-FE threshold over the same set.)
- **Honesty:** the folder is a *view convenience*, never a delete/archive. Done tickets remain in the ledger at the done stage; collapsing them changes nothing in state.
...
- **needs-you** per project and the count are already computed: `taskSummary.byStatus.needsYou` (detail view) and `listSummary().needsYou` (list view), both via `needsHumanDecision(ticket)` in `state.js` (hard-gate-rejected OR waiting-on-a-known-owner-with-no-heartbeat). The FE also has `ticketNeedsYou()` in `board.ts` for the per-card chip. The header **prefers the Core's canonical count `taskSummary.byStatus.needsYou`** (which also captures cases the client-side gate-only derivation cannot, e.g. a stalled-heartbeat owner) and **falls back to the client derivation — counting tickets where `ticketNeedsYou(t)` is true over the already-loaded `tickets`** only when `taskSummary.byStatus.needsYou` is absent. Either way it is a pure reduction over data already on the wire, not a per-ticket fetch.
...
| Active-segment accent | furthest in-progress stage, indexed in the **rendered rail** order (`backlog` + done stages dropped) over `tickets[].status` | **No** |
| Done folder (stacked, clickable) | done stage column, collapsed — `doneStage()` (done-name-first, else last) (§2) | **No** |
| Header needs-you / activity roll-up | canonical `taskSummary.byStatus.needsYou`, else `ticketNeedsYou` fallback / `ticket.active` (§3) | **No** |
...
| R2 | **Stage-token assumption** — hard-coding `"backlog"` / `"done"` onto tracks that don't name stages that way (Apex §2.2). | Med | Predicate is `unset/unknown OR == "backlog" OR a pre-start token not defined as a workflow stage`; the done stage is `doneStage()` = the first stage with a conventional done-name (`done`/`complete`/`completed`/`closed`/`shipped`/`released`), falling back to `last(stages)` when none match. It **degrades to "no Backlog bar / done-is-last-column"** when a track names neither — never mis-sorts, and a stage placed after done does not steal the folder. **Workflow-aware**: a pre-start token (`ready`/`triage`/…) that a track legitimately names as a stage routes to that stage's column, not Backlog. Generic help says "first/last stage," the real stage name shows on the column. |
