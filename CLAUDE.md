...
This is a **reusable AI agent-team framework** — 37 agent skills (a 15-agent core team + optional specialists, plus seven cross-cutting process skills including the `workflow-engine`), a proportional dev `workflow.yaml`, document templates, and the `install.sh` installer. Each skill file represents a team member with deep domain expertise. The skills are installed globally and used by every project. It is vendor-neutral and OSS-first, working in Claude Code / Cursor / Kiro / VS Code.
...
- **Skills**: 37 (15-agent core team + optional specialists + cross-cutting process skills; tech stacks as references)
