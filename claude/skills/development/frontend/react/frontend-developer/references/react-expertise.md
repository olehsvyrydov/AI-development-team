# Frontend — React/Next.js Expertise (TS · state · data · styling · forms · perf · testing · a11y)
...
#### Dual Translation Systems (Laravel + Inertia/Vue)
When working with Laravel + Inertia.js + vue-i18n:
- **Server-side** (Blade/Filament): `__('file.key')` reads from `lang/{locale}/*.php`
- **Client-side** (Vue/Inertia): `t('file.key')` reads from `resources/js/i18n/{locale}.js`

These are **independent systems**. Adding a key to PHP `lang/` does NOT make it available in vue-i18n. Always update both in the same commit when a feature spans server and client rendering.
